...
function set_posts_start() {
  const split_post_start=get_set_value('post_start');
...
function get_set_value(key) {
  ...
  const matching_location = window.location.search.match(regex);
...
function post_position_real() {
  const max_posts = document.getElementsByClassName('single_post_container').length;
...
function display_post_count(max_posts) {
  const current_post_page = parseInt(((posts_start_position+10)/10), 10);
  const post_pages_total = parseInt((max_posts/10), 10);
...
function update_url(new_param) {
  const new_key_value=new_param.split('=');
  if (window.location.search.search(new_key_value[0]) > -1) {
    const regex = new RegExp(new_key_value[0]+'=.+?(?=&|$)');
    const updated_location_search = window.location.search.replace(regex, new_param);
...
function pop_param_from_url(goner) {
  ...
  const updated_location_search = window.location.search.replace(regex, '');
  window.history.replaceState({}, '', updated_location_search);
}
...
function search_onLoad() {
  const search_term = get_set_value('search_term');
  if (search_term) {
...
function xhr_request(url) {
  return new Promise(function(resolve, reject) {
    const raw_json = new XMLHttpRequest();
